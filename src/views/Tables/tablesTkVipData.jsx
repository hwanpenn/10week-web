...
import {getOtherTkVipData,getDataTkVipData,updateDataTkVipData,deleteDataTkVipData,createDataTkVipData } from "actions/tablesTkVipData";
...
class tablesTkVipData extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            checked: [],
    ...
    getTableData = (articleTitle,start,size,userId) => {
        ...
        this.props.getDataTkVipData(params);
    }
    getOtherData = (username,start,size) => {
        ...
        this.props.getOtherTkVipData(params);
    }
    onRowSelect = (record) => {
    ...
    handleModify = () => {
        ...
        form.validateFields((err, values) => {
            ...
            this.props.updateDataTkVipData(values,this);
            // form.resetFields();
            // this.setState({ visibleModify: false });
        });
    }
    handleCreate = () => {
        ...
        form.validateFields((err, values) => {
            ...
            this.props.createDataTkVipData(values)
            form.resetFields();
            this.setState({ visible: false });
        });
    }
    ...
    deleteConfirm = (record) => {
        ...
        this.props.deleteDataTkVipData(params)
    }
    activeConfirm = (record) => {
        ...
        // this.props.activeDataTkVipData(params)
    }
    render() {
        let thisTemp = this
                    ...
                    <Card>
                        ...
                        <CardBody>
                            <Table onRow={(record) => {
                                ...
                                }} key={"tablesTkVipData"} pagination={false} columns={columns} dataSource={this.props.tablesTkVipData.tableDataTkVipData} scroll={{x:2240,  y: 360}} />
                            <LocaleProvider locale={zh_CN}>
                                <Pagination  current={this.state.current} showTotal={total => `总共 ${total} 条`} showSizeChanger showQuickJumper defaultPageSize={10} total={this.props.tablesTkVipData.tableCountTkVipData} style={{textAlign:'right',marginTop:25}}  onShowSizeChange={(current, pageSize)=>this.getTableData('',current, pageSize)} onChange={(page, pageSize)=>this.getTableData('',page,pageSize)}/>
                            </LocaleProvider>
                        </CardBody>
                    </Card>
                </GridItem>
...
const mapStateToProps = (state) => {
    return{
        tablesTkVipData: state.tablesTkVipData,
    }
}
const mapDispatchToProps = (dispatch) => {
    return{
        getDataTkVipData: (params) => {
            dispatch(getDataTkVipData(params))
        },
        updateDataTkVipData: (params,obj) => {
            dispatch(updateDataTkVipData(params,obj))
        },
        deleteDataTkVipData: (params) => {
            dispatch(deleteDataTkVipData(params))
        },
        createDataTkVipData: (params) => {
            dispatch(createDataTkVipData(params))
        },
        getOtherTkVipData: (params) => {
            dispatch(getOtherTkVipData(params))
        },
        // activeDataTkVipData: (params) => {
        //     dispatch(activeDataTkVipData(params))
        // }
    }
}

export default connect(mapStateToProps,mapDispatchToProps)(withStyles(extendedTablesStyle)(tablesTkVipData));
