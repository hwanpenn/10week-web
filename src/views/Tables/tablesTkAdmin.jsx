...
import {updatePasswordDataTkAdmin,getOtherTkAdmin,getDataTkAdmin,updateDataTkAdmin,deleteDataTkAdmin,createDataTkAdmin } from "actions/tablesTkAdmin";
...
class tablesTkAdmin extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            checked: [],
    ...
    getOtherData = (username,start,size) => {
        ...
        this.props.getOtherTkAdmin(params);
    }
    handleSearch = (value) => {
    ...
    handleModify = () => {
        ...
        form.validateFields((err, values) => {
            ...
            this.props.updateDataTkAdmin(values);
            form.resetFields();
            this.setState({ visibleModify: false });
        });
    }
    handleCreate = () => {
        ...
        form.validateFields((err, values) => {
            ...
            values.role = 'admin'
            this.props.createDataTkAdmin(values)
            form.resetFields();
            this.setState({ visible: false });
        });
    }
    ...
    deleteConfirm = (record) => {
        ...
        this.props.deleteDataTkAdmin(params)
    }
    resetConfirm = (record) => {
        const params = {
            id:record._id,
        }
        this.props.updatePasswordDataTkAdmin(params)
    }
    render() {
        let thisTemp = this
        ...
        let options = ''
        if(this.props.tablesTkAdmin.responseOtherTkAdmin.rows!==undefined){
             options = this.props.tablesTkAdmin.responseOtherTkAdmin.rows.map((item,i) => {
                    ...
                    <Card>
                        ...
                        <CardBody>
                            <Table onRow={(record) => {
                                ...
                                }} key={"tablesTkAdmin"} pagination={false} columns={columns} dataSource={this.props.tablesTkAdmin.tableDataTkAdmin} scroll={{x: 600, y: 360}} />
                            <LocaleProvider locale={zh_CN}>
                                <Pagination  current={this.state.current} showTotal={total => `总共 ${total} 条`} showSizeChanger showQuickJumper defaultPageSize={10} total={this.props.tablesTkAdmin.tableCountTkAdmin} style={{textAlign:'right',marginTop:25}}  onShowSizeChange={(current, pageSize)=>this.getTableData('',current, pageSize)} onChange={(page, pageSize)=>this.getTableData('',page,pageSize)}/>
                            </LocaleProvider>
                        </CardBody>
                    </Card>
                </GridItem>
...
const mapStateToProps = (state) => {
    return{
        tablesTkAdmin: state.tablesTkAdmin,
    }
}
const mapDispatchToProps = (dispatch) => {
    return{
        getDataTkAdmin: (params) => {
            dispatch(getDataTkAdmin(params))
        },
        updateDataTkAdmin: (params) => {
            dispatch(updateDataTkAdmin(params))
        },
        deleteDataTkAdmin: (params) => {
            dispatch(deleteDataTkAdmin(params))
        },
        createDataTkAdmin: (params) => {
            dispatch(createDataTkAdmin(params))
        },
        getOtherTkAdmin: (params) => {
            dispatch(getOtherTkAdmin(params))
        },
        updatePasswordDataTkAdmin: (params) => {
            dispatch(updatePasswordDataTkAdmin(params))
        }
    }
}

export default connect(mapStateToProps,mapDispatchToProps)(withStyles(extendedTablesStyle)(tablesTkAdmin));
