...
import {updatePasswordDataTkCoach,getOtherTkCoach,getDataTkCoach,updateDataTkCoach,deleteDataTkCoach,createDataTkCoach } from "actions/tablesTkCoach";
...
class tablesTkCoach extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            checked: [],
    ...
    getOtherData = (username,start,size) => {
        ...
        this.props.getOtherTkCoach(params);
    }
    handleSearch = (value) => {
    ...
    handleModify = () => {
        ...
        form.validateFields((err, values) => {
            ...
            this.props.updateDataTkCoach(values);
            form.resetFields();
            this.setState({ visibleModify: false });
        });
    }
    handleCreate = () => {
        ...
        form.validateFields((err, values) => {
            ...
            values.role = 'coach'
            this.props.createDataTkCoach(values)
            form.resetFields();
            this.setState({ visible: false });
        });
    }
    ...
    deleteConfirm = (record) => {
        ...
        this.props.deleteDataTkCoach(params)
    }
    resetConfirm = (record) => {
        const params = {
            id:record._id,
        }
        this.props.updatePasswordDataTkCoach(params)
    }
    render() {
        let thisTemp = this
        ...
        let options = ''
        if(this.props.tablesTkCoach.responseOtherTkCoach.rows!==undefined){
             options = this.props.tablesTkCoach.responseOtherTkCoach.rows.map((item,i) => {
                    ...
                    <Card>
                        ...
                        <CardBody>
                            <Table onRow={(record) => {
                                ...
                                }} key={"tablesTkCoach"} pagination={false} columns={columns} dataSource={this.props.tablesTkCoach.tableDataTkCoach} scroll={{x: 600, y: 360}} />
                            <LocaleProvider locale={zh_CN}>
                                <Pagination  current={this.state.current} showTotal={total => `总共 ${total} 条`} showSizeChanger showQuickJumper defaultPageSize={10} total={this.props.tablesTkCoach.tableCountTkCoach} style={{textAlign:'right',marginTop:25}}  onShowSizeChange={(current, pageSize)=>this.getTableData('',current, pageSize)} onChange={(page, pageSize)=>this.getTableData('',page,pageSize)}/>
                            </LocaleProvider>
                        </CardBody>
                    </Card>
                </GridItem>
...
const mapStateToProps = (state) => {
    return{
        tablesTkCoach: state.tablesTkCoach,
    }
}
const mapDispatchToProps = (dispatch) => {
    return{
        getDataTkCoach: (params) => {
            dispatch(getDataTkCoach(params))
        },
        updateDataTkCoach: (params) => {
            dispatch(updateDataTkCoach(params))
        },
        deleteDataTkCoach: (params) => {
            dispatch(deleteDataTkCoach(params))
        },
        createDataTkCoach: (params) => {
            dispatch(createDataTkCoach(params))
        },
        getOtherTkCoach: (params) => {
            dispatch(getOtherTkCoach(params))
        },
        updatePasswordDataTkCoach: (params) => {
            dispatch(updatePasswordDataTkCoach(params))
        }
    }
}

export default connect(mapStateToProps,mapDispatchToProps)(withStyles(extendedTablesStyle)(tablesTkCoach));
